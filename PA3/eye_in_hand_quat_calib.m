...
for i = 1:n
    ...
    E((1:3),4,i)= t_Robot_config(i,:)';                    %building the E and S transformation matrices
...
for k =1:(n-1)
    A_quat(:,:,k) = cvt_R2quat(A((1:3),(1:3),k));      %Quaternion representations of rotation matrices
...
R_quat = cvt_quat2R(Quat_rot)    %Converting to R_quat matrix
...
T_quat = inv(transpose(C_quat)*C_quat)*transpose(C_quat)*F_quat     %Solving using least squares approach
end